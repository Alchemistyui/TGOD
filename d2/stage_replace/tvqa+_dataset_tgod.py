# ------------------------------------------------------------------------
# TVQA+ Dataset with TGOD style input
# ------------------------------------------------------------------------
# Modified from STAGE (https://github.com/jayleicn/TVQAplus), tvqa_dataset.py
...
class TVQADatasetTextGuide(Dataset):
    def __init__(self, opt, mode="train"):
        ...
        if mode == "train":
            ...
            self.train_vfeat = torch.load(self.opt.train_feature_path)
            if self.opt.local_rank == 0:
                print('load train vcpt...')
            self.train_vcpt = torch.load(self.opt.train_vcpt_path)
        if mode != "test":
            if self.opt.local_rank == 0:
                print('load val feature...')
            self.val_vfeat = torch.load(self.opt.val_feature_path)
            if self.opt.local_rank == 0:
                print('load val vcpt...')
            self.val_vcpt = torch.load(self.opt.val_vcpt_path)
    ...
    def get_cur_dict(self):
        if self.mode == 'train':
            self.cur_vfeat = self.train_vfeat
            self.vcpt_dict = self.train_vcpt
            return self.raw_train
        elif self.mode == 'valid':
            self.cur_vfeat = self.val_vfeat
            self.vcpt_dict = self.val_vcpt
            return self.raw_valid
        elif self.mode == 'test':
            if self.opt.local_rank == 0:
                print('load test feature...')
            self.cur_vfeat = torch.load(self.opt.test_feature_path)
            if self.opt.local_rank == 0:
                print('load test vcpt...')
            self.vcpt_dict = torch.load(self.opt.test_vcpt_path)
    ...
    def __getitem__(self, index):
        ...
        if self.mode in ["test", "valid"] and self.vfeat_flag:
            items["boxes"] = self.vcpt_dict[qid]["boxes"][start_idx:end_idx+1]
            items["boxes"] = [[x[:self.num_region] for x in per_ans] for per_ans in items["boxes"]]
        else:
            items["boxes"] = None
        ...
        if self.vfeat_flag or self.vcpt_flag:
            region_counts = self.vcpt_dict[qid]["counts"][start_idx:end_idx+1] # full resolution
            # only select the one related to correct answers
            if "answer_idx" in self.cur_data_dict[index]:
                localized_lowered_region_counts = [min(x[ca_idx], self.num_region) for x in region_counts]

        # add vcpt
        if self.vcpt_flag:
            lower_res_obj_labels = []
            for obj in self.vcpt_dict[qid]["object"]:
                if len(obj) == 1:
                    obj = obj*5
                lower_res_obj_labels.append([x[:self.num_region] for x in obj])
        ...
        # add visual feature
        if self.vfeat_flag:
            items["vfeat"] = self.cur_vfeat[str(qid)]
            new_feat = []
            for x in items["vfeat"]:
                if torch.is_tensor(x):
                    new_feat.append(x[:, :self.num_region])
                else:
                    new_feat.append(torch.tensor(x).repeat(5,1,1)[:, :self.num_region])
            items["vfeat"] = new_feat
        else:
        ...
        if "answer_idx" in self.cur_data_dict[index] and self.use_sup_att and not self.inference and self.vfeat_flag:
            ...
            iou_data = self.get_iou_data(self.cur_data_dict[index]["bbox"], self.vcpt_dict[qid]["boxes"][start_idx:end_idx+1], frm_cnt, ca_idx)
    ...
    def get_iou_data(self, gt_box_data_i, meta_data_i, frm_cnt_i,ca_idx):
        """
        ...
        for idx, img_id in enumerate(img_ids):
            iou_data_i[img_id] = []
            cur_detected_boxes = meta_data_i[idx][ca_idx]
    ...
    def numericalize_hier_vcpt(self, vcpt_words_list):
        ...
        sentence_indices = []
        for per_img in vcpt_words_list:
            # some labels are 'tennis court', keep the later word
            img_words = []
            for per_ans in per_img:
                words = [e.split()[-1] for e in per_ans]
                img_words.append([self.word2idx[w] if w in self.word2idx else self.word2idx["<unk>"]
                                     for w in words])
            sentence_indices.append(img_words)
        return sentence_indices
...
def pad_multi_ans(sequences, dtype=torch.long):
    bsz = len(sequences)
    
    para_lengths = [len(seq) for seq in sequences]
    max_para_len = max(para_lengths)
    sen_lengths = [[[len(word_seq) for word_seq in ans_seq] for ans_seq in seq] for seq in sequences]
    max_sen_len = max(flat_list_of_lists(flat_list_of_lists(sen_lengths)))
    
    if isinstance(sequences[0][0], torch.Tensor):
        extra_dims = sequences[0][0].shape[2:]
    else:
        sequences = [[[torch.LongTensor(word_seq) for word_seq in ans_seq] for ans_seq in seq] for seq in sequences]
        extra_dims = ()

    padded_seqs = torch.zeros((bsz, max_para_len, 5, max_sen_len) + extra_dims, dtype=dtype)
    mask = torch.zeros(bsz, max_para_len, 5, max_sen_len).float()

    for b_i in range(bsz):
        for sen_i, ans_sen_l in enumerate(sen_lengths[b_i]):
            for ans_i, sen_l in enumerate(ans_sen_l):
                try:
                    padded_seqs[b_i, sen_i, ans_i, :sen_l] = sequences[b_i][sen_i][ans_i]
                except:
                    import pdb; pdb.set_trace()
                mask[b_i, sen_i, ans_i, :sen_l] = 1
    return padded_seqs, mask 
...
def pad_collate(data):
    ...
    batch["vcpt"], batch["vcpt_mask"] = pad_multi_ans([d["vcpt"] for d in data], dtype=torch.long)
    batch["vid"], batch["vid_mask"] = pad_multi_ans([d["vfeat"] for d in data], dtype=torch.float)
