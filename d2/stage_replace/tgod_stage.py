# ------------------------------------------------------------------------
# STAGE framework with TGOD style input
# ------------------------------------------------------------------------
# Modified from STAGE (https://github.com/jayleicn/TVQAplus), model/stage.py
...
import sys
sys.path.append('..')
from model.context_query_attention import StructuredAttention
from model.encoder import StackedEncoder
from model.cnn import DepthwiseSeparableConv
from model.model_utils import save_pickle, mask_logits, flat_list_of_lists, \
...
class STAGETextGuide(nn.Module):
    def __init__(self, opt):
        super(STAGETextGuide, self).__init__()
        self.opt = opt
    ...
    def forward_main(self, batch):
        ...
        if self.vfeat_flag:
            ...
            num_imgs, num_ans, num_regions = batch.vid.shape[1:4]
            vid_embed = F.normalize(batch.vid, p=2, dim=-1)  # (N, Li, Lans, Lr, D)
            vid_embed = self.base_encoder(vid_embed.view(bsz*num_imgs*num_ans, num_regions, -1),  # (N*Li, Lw)
                                          batch.vid_mask.view(bsz*num_imgs*num_ans, num_regions),  # (N*Li, Lr)
                                          self.vid_fc,
                                          self.input_embedding,
                                          self.input_encoder)  # (N*Li, L, D)
            
            vid_embed = vid_embed.contiguous().view(bsz, num_imgs, num_ans, num_regions, -1).permute(0,2,1,3,4)  # (N, 5, Li, Lr, D) 
            vid_mask = batch.vid_mask.permute(0,2,1,3)  # (N, 5, Li, Lr)
        ...
        att_loss = torch.tensor(0, device=out.device, dtype=torch.float32)
        ...
        for batch_idx in range(len(target)):  # batch
            ...
            for img_idx in available_img_indices:
                ...
                for word_idx in available_word_indices:
                    ...
                    if len(img_idx_word_idx_neg_indices) == 0:
                        continue
            ...
            for batch_idx in range(len(scores)):
                ...
                for ans_idx in range(5):
                    ...
                    for img_idx_local in range(len(boxes[batch_idx])):
                        ...
                        # scpres: (bs, #ans, #img, #word, #region)
                        cur_img_scores = scores[batch_idx, ans_idx, img_idx_global]  # (Lqa, Lr)
                        # words: (bs, #ans, #word)
                        cur_words = words[batch_idx, ans_idx].tolist()  # (Lqa, )
                        # boxes: (bs, #img, #ans, #region, 4)
                        cur_img_boxes = boxes[batch_idx][img_idx_local][ans_idx]
                        for word_idx, w in enumerate(cur_words):
                            if w in object_vocab:
                                ...
                                accepted_region_ids = np.nonzero(cur_word_region_scores[:len(cur_img_boxes)] >= score_thd)[0].tolist()
